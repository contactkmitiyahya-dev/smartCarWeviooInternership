...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  useEffect(() => {
    ...
    const distortions = {
      ...
      LongRaceDistortion: {
        ...
        getJS: (progress, time) => {
          const c = 0.0125, uFreq = LongRaceUniforms.uFreq.value, uAmp = LongRaceUniforms.uAmp.value;
          const d = new THREE.Vector3(
    ...
    const sideSticksFragment = `
      ...
      ${customFogParsFragment}
      varying vec3 vColor;
      void main(){ gl_FragColor=vec4(vec3(vColor),1.); ${customFogFragment} }`;
    ...
    class CarLights {
      ...
      init() {
        ...
        for (let i=0; i<options.lightPairsPerRoadWay; i++) {
          ...
          const carLane=i%options.lanesPerRoad, laneX0=carLane*laneWidth-options.roadWidth/2+laneWidth/2;
          const carWidth=random(options.carWidthPercentage)*laneWidth, carShiftX=random(options.carShiftX)*laneWidth;
          const laneX=laneX0+carShiftX, offsetY=random(options.carFloorSeparation)+radius*1.3, offsetZ=-random(options.length);
  ...
  return <div id="hyperspeed-bg" ref={hyperspeed}></div>;
};
