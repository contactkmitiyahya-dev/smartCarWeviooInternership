...
var customFogParsVertex = "\n  #ifdef USE_FOG\n  varying float vFogDepth;\n  #endif\n";
var customFogVertex = "\n  #ifdef USE_FOG\n  vFogDepth = -mvPosition.z;\n  #endif\n";
var customFogParsFragment = "\n  #ifdef USE_FOG\n  uniform vec3 fogColor;\n  uniform float fogNear;\n  uniform float fogFar;\n  varying float vFogDepth;\n  #endif\n";
var customFogFragment = "\n  #ifdef USE_FOG\n  float fogFactor = smoothstep(fogNear, fogFar, vFogDepth);\n  gl_FragColor.rgb = mix(gl_FragColor.rgb, fogColor, fogFactor);\n  #endif\n";
...
var Hyperspeed = function Hyperspeed(_ref) {
  var _ref$effectOptions = _ref.effectOptions,
    effectOptions = _ref$effectOptions === void 0 ? DEFAULT_EFFECT_OPTIONS : _ref$effectOptions;
  var hyperspeed = (0, _react.useRef)(null);
  var appRef = (0, _react.useRef)(null);
  var mergedOptions = (0, _react.useMemo)(function () {
    return _objectSpread(_objectSpread(_objectSpread({}, DEFAULT_EFFECT_OPTIONS), effectOptions), {}, {
      colors: _objectSpread(_objectSpread({}, DEFAULT_EFFECT_OPTIONS.colors), effectOptions.colors || {})
    });
  }, [effectOptions]);
  (0, _react.useEffect)(function () {
    if (appRef.current) {
      ...
      appRef.current = null;
      var _container = hyperspeed.current;
      if (_container) {
    ...
    var distortions = {
      mountainDistortion: {
        ...
        getJS: function getJS(progress, time) {
          var f = 0.02,
            uFreq = mountainUniforms.uFreq.value,
            uAmp = mountainUniforms.uAmp.value;
          var d = new THREE.Vector3(Math.cos(progress * Math.PI * uFreq.x + time) * uAmp.x - Math.cos(f * Math.PI * uFreq.x + time) * uAmp.x, nsin(progress * Math.PI * uFreq.y + time) * uAmp.y - nsin(f * Math.PI * uFreq.y + time) * uAmp.y, nsin(progress * Math.PI * uFreq.z + time) * uAmp.z - nsin(f * Math.PI * uFreq.z + time) * uAmp.z);
      ...
      xyDistortion: {
        ...
        getJS: function getJS(progress, time) {
          var f = 0.02,
            uFreq = xyUniforms.uFreq.value,
            uAmp = xyUniforms.uAmp.value;
          var d = new THREE.Vector3(Math.cos(progress * Math.PI * uFreq.x + time) * uAmp.x - Math.cos(f * Math.PI * uFreq.x + time) * uAmp.x, Math.sin(progress * Math.PI * uFreq.y + time + Math.PI / 2) * uAmp.y - Math.sin(f * Math.PI * uFreq.y + time + Math.PI / 2) * uAmp.y, 0);
      ...
      LongRaceDistortion: {
        ...
        getJS: function getJS(progress, time) {
          var c = 0.0125,
            uFreq = LongRaceUniforms.uFreq.value,
            uAmp = LongRaceUniforms.uAmp.value;
          var d = new THREE.Vector3(Math.sin(progress * Math.PI * uFreq.x + time) * uAmp.x - Math.sin(c * Math.PI * uFreq.x + time) * uAmp.x, Math.sin(progress * Math.PI * uFreq.y + time) * uAmp.y - Math.sin(c * Math.PI * uFreq.y + time) * uAmp.y, 0);
      ...
      turbulentDistortion: {
        ...
        getJS: function getJS(progress, time) {
          var uFreq = turbulentUniforms.uFreq.value,
            uAmp = turbulentUniforms.uAmp.value;
          var getX = function getX(p) {
            return Math.cos(Math.PI * p * uFreq.x + time) * uAmp.x + Math.pow(Math.cos(Math.PI * p * uFreq.y + time * (uFreq.y / uFreq.x)), 2) * uAmp.y;
          };
          var getY = function getY(p) {
            return -nsin(Math.PI * p * uFreq.z + time) * uAmp.z - Math.pow(nsin(Math.PI * p * uFreq.w + time / (uFreq.z / uFreq.w)), 5) * uAmp.w;
          };
          var d = new THREE.Vector3(getX(progress) - getX(progress + 0.007), getY(progress) - getY(progress + 0.007), 0);
      ...
      deepDistortion: {
        ...
        getJS: function getJS(progress, time) {
          var uFreq = deepUniforms.uFreq.value,
            uAmp = deepUniforms.uAmp.value,
            uPowY = deepUniforms.uPowY.value;
          var getX = function getX(p) {
            return Math.sin(p * Math.PI * uFreq.x + time) * uAmp.x;
          };
          var getY = function getY(p) {
            return Math.pow(p * uPowY.x, uPowY.y) + Math.sin(p * Math.PI * uFreq.y + time) * uAmp.y;
          };
          var d = new THREE.Vector3(getX(progress) - getX(progress + 0.01), getY(progress) - getY(progress + 0.01), 0);
    ...
    var random = function random(base) {
    ...
    var sideSticksFragment = "\n      #define USE_FOG;\n      ".concat(customFogParsFragment, "\n      varying vec3 vColor;\n      void main(){ gl_FragColor=vec4(vec3(vColor),1.); ").concat(customFogFragment, " }");
    ...
    var CarLights = /*#__PURE__*/function () {
      function CarLights(webgl, options, colors, speed, fade) {
        _classCallCheck(this, CarLights);
        this.webgl = webgl;
      ...
      return _createClass(CarLights, [{
        key: "init",
        value: function init() {
          var options = this.options;
          var curve = new THREE.LineCurve3(new THREE.Vector3(0, 0, 0), new THREE.Vector3(0, 0, -1));
          var geometry = new THREE.TubeGeometry(curve, 40, 1, 8, false);
          ...
          var aOffset = [],
            aMetrics = [],
            aColor = [];
          var colors = Array.isArray(this.colors) ? this.colors.map(function (c) {
            return new THREE.Color(c);
          }) : new THREE.Color(this.colors);
          for (var i = 0; i < options.lightPairsPerRoadWay; i++) {
            var radius = random(options.carLightsRadius),
              length = random(options.carLightsLength),
              speed = random(this.speed);
            var carLane = i % options.lanesPerRoad,
              laneX0 = carLane * laneWidth - options.roadWidth / 2 + laneWidth / 2;
            var carWidth = random(options.carWidthPercentage) * laneWidth,
              carShiftX = random(options.carShiftX) * laneWidth;
            var laneX = laneX0 + carShiftX,
              offsetY = random(options.carFloorSeparation) + radius * 1.3,
              offsetZ = -random(options.length);
    ...
    var LightsSticks = /*#__PURE__*/function () {
      function LightsSticks(webgl, options) {
        _classCallCheck(this, LightsSticks);
        this.webgl = webgl;
        this.options = options;
      }
      return _createClass(LightsSticks, [{
        key: "init",
        value: function init() {
          var options = this.options;
          ...
          var stickoffset = options.length / (options.totalSideLightSticks - 1);
          var aOffset = [],
            aColor = [],
            aMetrics = [];
          var colors = Array.isArray(options.colors.sticks) ? options.colors.sticks.map(function (c) {
            return new THREE.Color(c);
          }) : new THREE.Color(options.colors.sticks);
          for (var i = 0; i < options.totalSideLightSticks; i++) {
    ...
    var Road = /*#__PURE__*/function () {
      ...
      return _createClass(Road, [{
        key: "createPlane",
        value: function createPlane(side, width, isRoad) {
          var options = this.options;
          var geometry = new THREE.PlaneGeometry(isRoad ? options.roadWidth : options.islandWidth, options.length, 20, 100);
          var uniforms = {
            uTravelLength: {
              value: options.length
            },
    ...
    var App = /*#__PURE__*/function () {
      ...
      return _createClass(App, [{
        ...
        value: function initPasses() {
          ...
          this.bloomPass = new _postprocessing.EffectPass(this.camera, new _postprocessing.BloomEffect({
            luminanceThreshold: 0.2,
            luminanceSmoothing: 0,
            resolutionScale: 1
          }));
          var smaaPass = new _postprocessing.EffectPass(this.camera, new _postprocessing.SMAAEffect({
            preset: _postprocessing.SMAAPreset.MEDIUM,
            searchImage: _postprocessing.SMAAEffect.searchImageDataURL,
            areaImage: _postprocessing.SMAAEffect.areaImageDataURL
          }));
        ...
        value: function loadAssets() {
          ...
          return new Promise(function (resolve) {
            ...
            searchImage.src = _postprocessing.SMAAEffect.searchImageDataURL;
            areaImage.src = _postprocessing.SMAAEffect.areaImageDataURL;
          });
        }
      }, {
        key: "init",
        ...
        value: function update(delta) {
          var lerpPct = Math.exp(-(-60 * Math.log2(1 - 0.1)) * delta);
          ...
          var fovChange = lerp(this.camera.fov, this.fovTarget, lerpPct);
          if (fovChange !== 0) {
        ...
        value: function dispose() {
          ...
          if (this.scene) {
            this.scene.traverse(function (object) {
              ...
              if (object.material) {
                Array.isArray(object.material) ? object.material.forEach(function (m) {
                  return m.dispose();
          ...
          if (this.renderer) {
            ...
            if ((_this$renderer$domEle = this.renderer.domElement) !== null && _this$renderer$domEle !== void 0 && _this$renderer$domEle.parentNode) this.renderer.domElement.parentNode.removeChild(this.renderer.domElement);
          }
        ...
        key: "setSize",
        value: function setSize(width, height, updateStyles) {
          if (width <= 0 || height <= 0) {
            this.hasValidSize = false;
            return;
          }
    ...
    var container = hyperspeed.current;
    if (!container) return;
    var options = _objectSpread({}, mergedOptions);
    options.distortion = distortions[options.distortion] || distortions.turbulentDistortion;
    var myApp = new App(container, options);
...
var _default = exports["default"] = Hyperspeed;